...
function acas() {
    ...
    var sden = new Array("Воскресенье", "Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота");
    var smesic = new Array("января", "февраля", "марта", "апреля", "мая", "июня", "июля", "августа", "сентября", "октября", "ноября", "декабря");
    var jom = new Array("Ришон", "Шени", "Шлиши", "Ревии", "Хамиши", "Шиши", "Шабат");
	
    if (hodina<4) {
        atext = "Спокойной ночи полушарию погруженному сейчас в темноту";
    }
    if (hodina>=4&&hodina<7) {
        atext = "Спокойной ночи и сладких снов";
    }
    if (hodina>=4&&hodina<7) {
        atext = "Доброе утро всем утренним птицам";
    }
    if (hodina>=7&&hodina<9) {
        atext = "Доброе утро и приятный день";
    }
    if (hodina>=9&&hodina<12) {
        atext = "Доброе утро всем посетителям";
    }
    if (hodina>=12&&hodina<13) {
        atext = "Доброго полдня всем и вкусного обеда :-)";
    }
    if (hodina>=13&&hodina<17) {
        atext = "Доброго дня всем хорошим людям";
    }
    if (hodina>=17&&hodina<19) {
        atext = "Приятного заката";
    }
    if (hodina>=19&&hodina<22) {
        atext = "Добрый вечер";
    }
    if (hodina<0) {
        atext = "Приятного вечера всем, кто еще не спит";
    }
    ...
    dnes += smesic[mesic] + "(";
    dnes += 1 + mesic + "),</b> ";
    dnes += "неделя но. <b>" + ctydnu + "</b>, ";
    dnes += "год <b>" + rok + "</b>, ";
    dnes += "точное время: <b>" + pcas + "</b>, ";
    ...
    document.getElementById("hajom").innerHTML = "Еврейский календарь: <b>йом " + hajom + "</b>";
    ...
    function FormatDateH(cDate) {
        ...
        switch (Number(aDate[0])) {
          case 1:
            cFormatDate = "Тишреи(1)";
            break;

          case 2:
            cFormatDate = "Хешван(2)";
            break;

          case 3:
            cFormatDate = "Кислев(3)";
            break;
          ...
          case 6:
            cFormatDate = "Адар A(6)";
            break;

          case 7:
            cFormatDate = IsLeapYear(Number(aDate[2])) ? "Адар B(7)" : "Адар(7)";
            break;
          ...
          case 10:
            cFormatDate = "Сиван(10)";
            break;

          case 11:
            cFormatDate = "Тамуз(11)";
            break;
